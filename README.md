...
두 모드 모두 구조는 동일 — 루트에 `waymark/<status>/`. 자세히는 [ARCHITECTURE.md](./ARCHITECTURE.md) §12.
...
위 결론들의 정식 이름 — 설계 근거는 [ARCHITECTURE.md](./ARCHITECTURE.md).
...
- **vs OpenSpec** — OpenSpec은 유지비를 내고 읽을 수 있는 현재상태 spec을 얻고, Waymark은
  그 spec을 포기해 드리프트를 없앤다. **트레이드오프.** → [ARCHITECTURE.md](./ARCHITECTURE.md) §0.
...
- [x] **v0.1** — 규약 · `/work-new` · **ambient 규칙**(자동 이동) · 게이트(`waymark-index`/`waymark-check`) · `waymark-init`·`waymark-remove` 스킬
- [ ] **v0.2** — 링크 liveness · 코드심볼 resolve · contract 테스트 · index merge-driver · CI 워크플로우

## 더 읽기

- [ARCHITECTURE.md](./ARCHITECTURE.md) — 포지셔닝(vs OpenSpec)·폴더 모델·id/스키마·배포 모드·강제력 등 설계 결정 전부.

## 이름

**Waymark** — 등산로에 **얇게** 쌓아 다음 갈림길을 가리키는 돌탑·표식(cairn)에서 왔다.
기념비가 아니라 **길을 잇는 최소한의 흔적**이라는 점이 이 도구가 하는 일과 정확히 겹친다
— 슬로건 "*Leave a marker, not a monument*" 그대로다. `way`(경로·워크플로우) + `mark`
(git이 커밋을 찍듯 이슈를 찍어 표시)의 결합이기도 하다.
