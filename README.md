...
전부 정당한 방식이고 각자 **진짜 강점**이 있다. "낫다/못하다"가 아니라 서로 **다른 세금**을 낸다.
...
| **Jira / Linear** | 조직 전체 · 비개발자 접근 · 성숙 | 프로젝트 관리의 원천 (**Docat이 보완**) |
| **Docat** | 복제 안 함(드리프트 0) · 트래커 보완 · git-native | **기획이 이미 밖(Confluence)**, 계약이 코드에 |

Docat이 채우는 틈은 *"진실이 이미 다른 곳에 있는데 그걸 또 복제하기는 싫은"* 특정한 경우다.
그 전제가 없다면 — greenfield거나, 읽을 수 있는 in-repo spec을 원한다면 — **Spec Kit·OpenSpec이
더 맞고, 우리도 그렇게 권한다.** 자세한 트레이드오프는 [DESIGN.md](./DESIGN.md) §0.
