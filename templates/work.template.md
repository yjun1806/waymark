<!--
  Docat issue doc — ONE ISSUE = ONE THIN FILE.
  Principles:
  ...
  Setup:
   - Copy to docat/draft/<id>-<slug>.md, then git mv between status folders as it moves:
   ...
   - id: tracker id (JIRA-123) if you have a tracker; else <PREFIX>-<seq> from the
     .docat.yml roster (e.g. YJ-6). Never a shared global counter.
   ...
   - Language: title / summary / body are authored in your team's language
     (.docat.yml `lang`: en for English teams, ko for Korean, …). Only the slug is ASCII.
...
-->
---
id: <id>                    # JIRA-123 (tracker) or YJ-6 (.docat.yml roster: <prefix>-<seq>)
