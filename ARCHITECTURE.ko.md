...
### 왜 필드가 아니라 폴더인가 (path-native)

상태를 **파일 내용**(frontmatter `status:`)에 두면 에이전트는 파일을 **읽어야** 상태를 안다.
grep 히트가 `done/` 문서에서 나면, 레거시인 줄 모르고 전문을 컨텍스트에 올린 뒤에야 "done이네,
버리자"가 된다 — 순수 낭비다. 폴더=상태면 상태가 **파일시스템 레벨의 경로**라, `grep -r
waymark/in-progress/` 또는 `--exclude-dir=done`으로 **읽지 않고 경로로 배제**한다(토큰 0).

- `done/`은 동결이라 **영원히 누적**되므로, 레거시가 쌓일수록 이 이점이 커진다(§8 done 캐시와
  같은 계열 — 누적된 `done/`의 비용을 0에 수렴시킨다).
- 단서: 에이전트가 실제로 scope를 걸어야 실현된다(통짜 `grep -r waymark/`는 done도 읽음). 경로
  배제는 자명·저렴하지만 내용 기반 상태 필터는 번거롭다 — 구조가 "싼 길"을 자명하게 만들 뿐 강제하진 않는다.
- 전제: grep/glob 같은 **경로 기반 검색**에 해당. 시맨틱 인덱스엔 덜 적용되나, Claude Code의
  지배적 검색이 경로 기반이라 현 환경에 정확히 맞는다.

요약: **상태는 내용이 아니라 경로다 — 그래서 에이전트가 안 읽고도 거른다.**
